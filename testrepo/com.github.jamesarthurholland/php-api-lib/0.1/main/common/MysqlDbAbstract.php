...
abstract class MysqlDbAbstract {
    ...
    protected $port = 80;
    protected $user = "root";
    protected $pass = "Corona420!";
    protected $dbName  = "{{ROOT_NAMESPACE}}";
    protected $indexKey = "id";
    public $link;
...
//        $this->connect($this->getDbName());
    ...
    {
        $this->connect($this->getDbName());
        $query = $this->getLink()->prepare("SELECT * FROM " . $this->getTable() . " WHERE " . $this->indexKey . " = ? ");
    ...
    public function getColumnNamesArray($entity)
    {
        $columns = [];
        foreach ($entity->toArray() as $key => $value) {
//            $columnName = implode("_", preg_split('/(?=[A-Z])/', $key));
...
//            echo "shiftDb save";
            ...
            foreach ($this->getColonPrefixedNamesArray($entity) as $colonPrefixedVar) {
                ...
                if(is_callable(array($entity, $getVariable))){
                    $variable = $entity->{$getVariable}();
                    $query->bindValue($colonPrefixedVar, $variable);
//                    echo '<p>ColonPrefixed=' . $colonPrefixedVar . ' , getter is ' . $getVariable .  ' ,  var is' . $variable . '</p>';
                }else {
                    echo "no variable " . $getVariable . " on entity. \n";
                    var_dump($entity);
                }
